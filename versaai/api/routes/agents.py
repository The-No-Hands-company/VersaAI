...
Endpoints:
    POST /v1/agents/execute    — Run a task on a named agent
...
async def execute_agent(request: AgentExecuteRequest):
    ...
    logger.info(f"[{request_id}] Agent={agent_name} Task={request.task[:100]}...")
    ...
    try:
        # Dispatch based on agent type — run blocking agent calls in a thread
        ...
        if agent_name == "coding":
            ...
            # Map human-friendly strategy names to enum values
            _STRATEGY_ALIASES = {
